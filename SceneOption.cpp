...
void SceneOption::updateItemList() {
    ...
    // Index 10: JUDGE OFFSET
    items.emplace_back("[ JUDGE OFFSET (ms) ]", getValStr(std::to_string(Config::JUDGE_OFFSET), 10));
    // Index 11: VISUAL OFFSET
    items.emplace_back("[ VISUAL OFFSET (px) ]", getValStr(std::to_string(Config::VISUAL_OFFSET), 11));

    // Index 12: MORE NOTES COUNT
    items.emplace_back("[ MORE NOTES COUNT ]", getValStr(std::to_string(Config::MORE_NOTES_COUNT), 12));

    // Index 13: Section
    items.emplace_back("--- [ EFFECT SETTINGS ] ---", "");
    // Index 14: BOMB DURATION
    items.emplace_back("[ BOMB DURATION (ms) ]", getValStr(std::to_string(Config::BOMB_DURATION_MS), 14));
    // Index 15: BOMB SIZE
    items.emplace_back("[ BOMB SIZE ]", getValStr(std::to_string(Config::BOMB_SIZE_FACTOR), 15));

    // Index 16: Section
    items.emplace_back("--- [ FOLDER SETTINGS ] ---", "");
    // Index 17-22: 仮想フォルダ設定項目
    items.emplace_back("[ FOLDER: LEVEL ]", getValStr((Config::SHOW_LEVEL_FOLDER ? "ON" : "OFF"), 17));
    items.emplace_back("[ FOLDER: LAMP ]", getValStr((Config::SHOW_LAMP_FOLDER ? "ON" : "OFF"), 18));
    items.emplace_back("[ FOLDER: RANK ]", getValStr((Config::SHOW_RANK_FOLDER ? "ON" : "OFF"), 19)); 
    items.emplace_back("[ FOLDER: TYPE ]", getValStr((Config::SHOW_CHART_TYPE_FOLDER ? "ON" : "OFF"), 20));
    items.emplace_back("[ FOLDER: NOTES ]", getValStr((Config::SHOW_NOTES_RANGE_FOLDER ? "ON" : "OFF"), 21));
    items.emplace_back("[ FOLDER: ALPHA ]", getValStr((Config::SHOW_ALPHA_FOLDER ? "ON" : "OFF"), 22));
...
OptionState SceneOption::update(SDL_Renderer* ren, NoteRenderer& renderer) {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        if (e.type == SDL_JOYBUTTONDOWN) {
            ...
            if (state == OptionState::SELECTING_ITEM) {
                ...
                else if (cursor == 10) { // JUDGE OFFSET
                ...
                else if (cursor == 11) { // VISUAL OFFSET
                ...
                else if (cursor == 12) { // MORE NOTES COUNT
                ...
                else if (cursor >= 17 && cursor <= 22) { // FOLDERS
                    if (btn == Config::SYS_BTN_LEFT || btn == Config::SYS_BTN_RIGHT || btn == Config::SYS_BTN_UP || btn == Config::SYS_BTN_DOWN) {
                        if (cursor == 17) Config::SHOW_LEVEL_FOLDER = !Config::SHOW_LEVEL_FOLDER;
                        else if (cursor == 18) Config::SHOW_LAMP_FOLDER = !Config::SHOW_LAMP_FOLDER;
                        else if (cursor == 19) Config::SHOW_RANK_FOLDER = !Config::SHOW_RANK_FOLDER;
                        else if (cursor == 20) Config::SHOW_CHART_TYPE_FOLDER = !Config::SHOW_CHART_TYPE_FOLDER;
                        else if (cursor == 21) Config::SHOW_NOTES_RANGE_FOLDER = !Config::SHOW_NOTES_RANGE_FOLDER;
                        else if (cursor == 22) Config::SHOW_ALPHA_FOLDER = !Config::SHOW_ALPHA_FOLDER;
                    }
                    else if (btn == Config::SYS_BTN_DECIDE || btn == Config::SYS_BTN_BACK) state = OptionState::SELECTING_ITEM;
                    else changed = false;
                }
    ...
    if (state == OptionState::SELECTING_ITEM || (state == OptionState::ADJUSTING_VALUE && cursor != 6 && cursor != 7)) {
        ...
        if (state == OptionState::ADJUSTING_VALUE) {
            ...
            renderer.drawText(ren, (cursor == 4 || cursor == 5 || cursor == 10 || cursor == 11 || cursor == 14 || cursor == 15 ? "SYS-UP/DOWN: +-5  SYS-L/R: +-1" : cursor == 12 ? "SYS-UP/DOWN: +-10 (0-200)" : "SYS-UDLR TO SWITCH TYPE"), 640, 670, {120, 120, 120, 255}, true, true);
        } else if ((cursor >= 4 && cursor <= 15) || (cursor >= 17 && cursor <= 22)) {
